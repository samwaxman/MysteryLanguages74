...
(define (same-type? t1 t2 instance-types)
  (cond
    [(and (nominal-struct-type? t1) (nominal-struct-type? t2))
     (same-type? (get-instance-type instance-types t1) (get-instance-type instance-types t2) instance-types)]
    [(and (struct-type? t1) (struct-type? t2))
     (equal? (struct-type-args t1) (struct-type-args t2))]
    [else (or (equal? t1 t2) (anything? t1) (anything? t2) (nominal-struct-type-equal? t1 t2)
              (func-equal? t1 t2 (lambda (t1 t2) (same-type? t1 t2 instance-types))))]))

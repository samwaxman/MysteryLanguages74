...
;Reassigns a variable in a singular given scope.
;Returns the updated scope if successful and false if not
(define (#%reassign-in-scope scope var new-value)
  (cond
    [(null? scope) (list #f)]
    [(cons? scope) (if (equal? (vector-ref (car scope) 0) var)
                       (cons (vector var new-value) (cdr scope))
                       (cons (car scope) (#%reassign-in-scope (cdr scope) var new-value)))]))

;Reassigns to the closest visible var with the right name
(define-syntax-rule (#%reassign var new-value)
  (let ([eval-new-val new-value])
    (reassign 'var eval-new-val)
    eval-new-val))
...
(define-syntax-rule (my-let ([id binding]) body ... last-body)
  (let () (begin-scope) (push id binding) (let ([result (begin body ... last-body)])
                                            (exit-scope) result)))
...
(define-syntax-rule (my-module-begin body ...)
  (Cond-#%module-begin
   (set-env '())
   (begin-scope)
   (let ([result (my-let ([recap Cond-recap])
